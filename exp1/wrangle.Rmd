...
This file wrangles raw data from Exp1 of the morality aesthetics project, which involved rating data (on likert scales). It produces some summary data plots, saves out data files for modelling and further analysis in later scripts.
...
datal <- dataf %>%
  ...
  mutate(image_cat = if_else(str_starts(item, "L"), "landscape", "people"),
         ...
         item = factor(item),
         group = if_else(group == "noInfo", "none", group),
         group = factor(group, levels = c("none", "info")),
         dv = case_match(dv, 
                         "AIUse" ~ "use", 
                         "FinanGain" ~ "gain",
                         "Prestige" ~ "prestige", 
...
```{r}
datad <- dataw %>%
  mutate(group = if_else(group == "none", -0.5, 0.5),
