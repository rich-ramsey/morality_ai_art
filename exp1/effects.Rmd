...
bf.2 <- readRDS("exp1/models/bf.2.rds")
```
...
ggsave ("exp1/figures/ppreds_line.jpeg",
        width = 8, height = 6)
```
...
```{r}
summary_d_fit_alt <- d_fit_alt %>%
  summarise(mean = mean(sum2))
head(summary_d_fit_alt)
```

plot

```{r}
p_pred <- ggplot(summary_d_fit_alt, aes(x = mean, y = fct_rev(response), fill=group)) +  
  geom_vline(xintercept = 0, color = "grey", alpha = 5/10) +
  stat_halfeye(point_size = 1) +
  labs(title = "Posterior predictions by group and image_type",
       x = "response", y = NULL) +
  scale_fill_brewer(palette="Dark2") +
  theme(panel.grid   = element_blank(),
        panel.grid.major.y = element_line(color = alpha("firebrick4", 1/2), linetype = 3),
        axis.text.y  = element_text(hjust = 0),
        axis.ticks.y = element_blank()) +
  facet_wrap(~image_type, ncol = 2)
p_pred

ggsave ("exp1/figures/ppreds.jpeg",
        width = 8, height = 6)
```

## calculate difference scores based on group ##

wrangle 

```{r}
d_fit_diff <- d_fit_alt %>%
  summarise(mean = mean(sum2)) %>% 
...
```{r}
p_pred_diff <- ggplot(d_fit_diff, aes(x = diff, y = fct_rev(response), fill=response)) +  
  geom_vline(xintercept = 0, color = "grey", alpha = 5/10) +
  stat_halfeye(point_size = 1) +
  labs(title = "Posterior predictions for the difference between group (no info > info)",
       x = NULL, y = NULL) +
  scale_fill_brewer(palette="Dark2") +
  theme(panel.grid   = element_blank(),
        panel.grid.major.y = element_line(color = alpha("firebrick4", 1/2), linetype = 3),
...
ggsave ("exp1/figures/ppreds_diff.jpeg",
        width = 8, height = 6)
```

calculate quantiles on the difference scores

```{r}
pred_diff_q <- d_fit_diff %>%
  group_by(image_type, response) %>% 
  median_qi(diff) %>% 
  select(image_type, response, diff, .lower, .upper) %>% 
  mutate(across(where(is.double), \(x) round(x, 2)))
pred_diff_q
```
...
```{r}
p_pred_tab <- flextable(pred_diff_q)
p_pred_tab

## save it
save_as_image(p_pred_tab, path = "exp1/tables/ppred_diff.png")
```

Maybe a dumb question, but why do the posterior preds look like the raw data (e.g., differences by group) more so than the fixed effects of group in the model output (where only one dv has a 95% interval non-overlapping with zero ??)??.
